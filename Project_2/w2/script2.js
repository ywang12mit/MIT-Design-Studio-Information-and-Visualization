...
var blue = canvas.append("g")
    ...
    .attr("fill", "none")
    .attr("stroke-width", 10*data.currently.humidity)
    ...
    .datum(function(d, i) {
      ...
      return d3.radialLine()
          ...
          .radius(function(a) {
            ...
            return 200 //200
                    + (1+data.currently.precipProbability*100)*(Math.cos(a * 4 - i * 2 * Math.PI / 3 + t) 
...
var red = canvas.append("g")
    .attr("transform", "translate(" + width / 2 + "," + height / 2 + ")") //circle position
    .attr("fill", "none")
    .attr("stroke-width", 10*data.currently.cloudCover)
    .attr("stroke-linejoin", "round") // rounded lines, not sharp
    ...
    .datum(function(d, i) {
      ...
      return d3.radialLine()
          ...
          .radius(function(a) {
            var t = d3.now() / 1000;
            return 200 //200
                    + data.currently.windSpeed/5.5*Math.cos(a * 4 - i * 2 * Math.PI / 3 + t) //a =8
                    * Math.pow((1 + Math.cos(a - t)) / 2, 3) 
                    * 32;
          });//a-->the bigger the denser
            //i&t --> the delay between lines 1-60
...
var yellow = canvas.append("g")
    .attr("transform", "translate(" + width / 2 + "," + height / 2 + ")") //circle position
    .attr("fill", "none")
    .attr("stroke-width", 5*data.currently.uvIndex+5)
    .attr("stroke-linejoin", "round") // rounded lines, not sharp
    ...
    .datum(function(d, i) {
      ...
      return d3.radialLine()
          ...
          .radius(function(a) {
            var t = d3.now() / 1000;
            return 200 //200
                    + data.currently.temperature/15*Math.cos(a * 4 - i * 2 * Math.PI / 3 + t) 
                    * Math.pow((1 + Math.cos(a - t)) / 2, 3) 
                    * 32;
          });//a-->the bigger the denser
            //i&t --> the delay between lines 1-60
